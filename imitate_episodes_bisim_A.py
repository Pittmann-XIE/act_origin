...
from utils_bisim_A import load_data # data functions
from utils_bisim_A import sample_box_pose, sample_insertion_pose # robot functions
from utils_bisim_A import compute_dict_mean, set_seed, detach_dict # helper functions
from policy_bisim_A import ACTPolicy, CNNMLPPolicy
...
def main(args):
    ...
    device = args['device'] # ADDED DEVICE
    ...
    if policy_class == 'ACT':
        ...
        policy_config = {'lr': args['lr'],
                         ...
                         'use_a_hat': args.get('use_a_hat', False),
                         'num_jump_samples': args.get('num_jump_samples', 3),
                         'decay_gamma':args.get('decay_gamma', 0.9)
    ...
    config = {
        ...
        'device': device # ADDED DEVICE TO CONFIG
    }
...
def get_image(ts, camera_names, device='cuda'): # ADDED DEVICE
    curr_images = []
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    device = config['device'] # ADDED DEVICE
    onscreen_cam = 'angle'
    ...
    policy.to(device) # REPLACED .cuda() with .to(device)
    policy.eval()
    ...
    for rollout_id in range(num_rollouts):
        ...
        with torch.inference_mode():
            for t in range(max_timesteps):
                # --- MODIFICATION START: MUJOCO LIGHTING ---
                if not real_robot:
                    update_mujoco_lighting(env, t, mode=None)
                    # We must re-render the images in `ts` so the policy sees the new lighting
                    for cam_name in camera_names:
                ...
                qpos_history[:, t] = qpos
                curr_image = get_image(ts, camera_names, device) # PASS DEVICE
                ...
                raw_action = raw_action.squeeze(0).cpu().numpy() # .cpu() remains as we need numpy array
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    if 'cuda' in device and not torch.cuda.is_available():
        print("⚠️  Warning: CUDA requested but no GPUs found. Falling back to CPU.")
        device = 'cpu'
    # -----------------------------
    ...
    # Initialize optimizer BEFORE wrapping in DataParallel
    optimizer = make_optimizer(policy_class, policy)

    # --- MULTI-GPU SETUP ---
    is_multi_gpu = device != 'cpu' and torch.cuda.device_count() > 1
    if is_multi_gpu:
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.inference_mode():
            ...
            if epoch_val_loss < min_val_loss:
                ...
                state_to_save = policy.module.state_dict() if is_multi_gpu else policy.state_dict()
                best_ckpt_info = (epoch, min_val_loss, deepcopy(state_to_save))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', action='store', type=str, default='cuda', help='Device to use (e.g. cuda:0, cuda:1, cpu)')
    ...
    parser.add_argument('--bisim_weight', action='store', type=float, default=0.5, help='Bisimulation loss weight', required=False)
    parser.add_argument('--use_a_hat', action='store_true', default=False)
    parser.add_argument('--num_jump_samples', action='store', type=int, default=10, help='bisim sample number', required=False)
    parser.add_argument('--decay_gamma', action='store', type=float, default=0.9, help='bisim weight decay', required=False)
