...
from detr.models_variant_b.detr_vae import build as build_variant_b_model


def build_variant_b_model_and_optimizer(args_override):
    parser = argparse.ArgumentParser("Variant B DETR training", parents=[get_args_parser()])
    args, _ = parser.parse_known_args()
    for key, value in args_override.items():
        setattr(args, key, value)
    print("args: ", args)

    model = build_variant_b_model(args)
...
class ACTPolicy(nn.Module):
    def __init__(self, args_override):
        super().__init__()
        model, optimizer = build_variant_b_model_and_optimizer(args_override)
        self.model = model
    ...
    def set_train_stage(self, stage):
        ...
        for name, param in self.model.named_parameters():
            if self._is_ema_parameter(name):
                ...
                param.requires_grad = self._is_future_parameter(name)
