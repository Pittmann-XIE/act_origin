...
BOX_POSE = [None] # to be changed from outside
ENABLE_DISTRACTOR = [True] # Controls if the distractor is in view
DISTRACTOR_SHAPES = [None, None] # Synchronizes shapes across EE and Joint sim
    ...
    def initialize_episode(self, physics):
        ...
        with physics.reset_context():
            ...
            if ENABLE_DISTRACTOR[0]:
                ...
                # Safer clearance threshold to prevent larger geoms from spawning inside the box
                clearance = 0.085 
                
                # --- Position Distractor 1 ---
                dist1_x, dist1_y = red_x, red_y
                while np.sqrt((dist1_x - red_x)**2 + (dist1_y - red_y)**2) < clearance:
                    dist1_x = np.random.uniform(0.0, 0.2)
                    dist1_y = np.random.uniform(0.4, 0.6)
                ...
                dist2_x, dist2_y = red_x, red_y
                # Check distance against BOTH the target box and distractor 1
                while (np.sqrt((dist2_x - red_x)**2 + (dist2_y - red_y)**2) < clearance) or \
                      (np.sqrt((dist2_x - dist1_x)**2 + (dist2_y - dist1_y)**2) < clearance):
                    dist2_x = np.random.uniform(0.0, 0.2)
                    dist2_y = np.random.uniform(0.4, 0.6)
                ...
                # --- Randomize Distractor Shapes ---
                if DISTRACTOR_SHAPES[0] is not None:
                    # Using raw mjGEOM integer values for absolute dm_control compatibility
                    shape_map = {
                        'sphere': (2, [0.02, 0, 0]),
                        'capsule': (3, [0.015, 0.02, 0]),
                        'cylinder': (4, [0.015, 0.02, 0]),
                        'ellipsoid': (6, [0.015, 0.02, 0.015])
                    }
                    
                    g1_id = physics.model.name2id('distractor_1_geom', 'geom')
                    g2_id = physics.model.name2id('distractor_2_geom', 'geom')
                    
                    t1, s1 = shape_map[DISTRACTOR_SHAPES[0]]
                    t2, s2 = shape_map[DISTRACTOR_SHAPES[1]]
                    
                    # Update shape types
                    physics.model.geom_type[g1_id] = t1
                    physics.model.geom_type[g2_id] = t2
                    
                    # Update shape sizes
                    physics.model.geom_size[g1_id] = s1
                    physics.model.geom_size[g2_id] = s2
...
class InsertionTask(BimanualViperXTask):
    ...
    def get_reward(self, physics):
        ...
        if touch_left_gripper and touch_right_gripper: 
            reward = 1
        if touch_left_gripper and touch_right_gripper and (not peg_touch_table) and (not socket_touch_table): 
            reward = 2
