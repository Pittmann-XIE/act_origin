...
from utils_z import load_data # data functions
from utils_z import sample_box_pose, sample_insertion_pose # robot functions
from utils_z import compute_dict_mean, set_seed, detach_dict # helper functions
from policy_z import ACTPolicy, CNNMLPPolicy
...
def forward_pass(data, policy, device='cuda'): 
    # Handle the new 6-item tuple from utils_track.py:
    # (image_data, qpos_data, action_data, is_pad, box_tensor, gaze_data)
    if len(data) == 6:
        image_data, qpos_data, action_data, is_pad, box_data, gaze_data = data
        
        image_data = image_data.to(device)
        qpos_data = qpos_data.to(device)
        action_data = action_data.to(device)
        is_pad = is_pad.to(device)
        gaze_data = gaze_data.to(device) # Move gaze to GPU
        
        return policy(qpos_data, image_data, action_data, is_pad, gaze_data=gaze_data)
        
    elif len(data) == 5:
        # Fallback if gaze is disabled
        image_data, qpos_data, action_data, is_pad, box_data = data
        image_data, qpos_data, action_data, is_pad, box_data = image_data.to(device), qpos_data.to(device), action_data.to(device), is_pad.to(device), box_data.to(device)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.inference_mode():
            ...
            for batch_idx, data in enumerate(val_dataloader):
                forward_dict = forward_pass(data, policy, device) # PASS DEVICE
...
if __name__ == '__main__':
    ...
    parser.add_argument('--z_weight', action='store', type=float, default=1.0, help='Weight for gaze loss')
