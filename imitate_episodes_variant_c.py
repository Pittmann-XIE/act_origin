...
"""
Multi-stage training notes (Variant C):
...
- `three_stage`: stage order `act` -> `future` -> `joint`.
    ...
    * Stage 2 (`future`): train the action-conditioned future visual simulator
        (multi-horizon future RGB and latent prediction present in Variant C).
    * Stage 3 (`joint`): fine-tune all components together.

- Implementation details:
    * `policy.set_train_stage(stage)` in `policy_variant_c.py` toggles
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    if policy_class != "ACT":
        raise NotImplementedError("Variant C future visualization is implemented for ACT policy only.")
    if real_robot:
        raise NotImplementedError("Variant C future visualization currently supports simulation only.")
...
def train_bc_three_stage(train_dataloader, val_dataloader, config):
    ...
    print(
        "\nStarting three-stage Variant C training: "
