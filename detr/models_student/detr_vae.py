...
class DETRVAE(nn.Module):
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, train_stage=1):
        ...
        if self.backbones is not None:
            ...
            # Teacher: All cameras concatenated along width
            src_teacher = torch.cat(all_cam_features, axis=3)
            pos_teacher = torch.cat(all_cam_pos, axis=3)
            
            # Student: Only camera 0
            ...
            if train_stage == 1:
                ...
                hs, attn_weights = self.transformer(
                    ...
                    latent_input, proprio_input, self.additional_pos_embed.weight,
                    src_student=src_student, pos_student=pos_student, train_stage=0
                )
        else:
            # Fallback for state-only
