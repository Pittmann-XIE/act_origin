...
# Replace the Transformer class in transformer.py
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, latent_input=None, proprio_input=None, 
                additional_pos_embed=None, src_student=None, pos_student=None, train_stage=1):
        ...
        if src is not None:
            ...
            src_t = src.flatten(2).permute(2, 0, 1)
            pos_t = pos_embed.flatten(2).permute(2, 0, 1).repeat(1, bs, 1)
            pos_t = torch.cat([additional_pos_embed.unsqueeze(1).repeat(1, bs, 1), pos_t], axis=0)
            src_t = torch.cat([torch.stack([latent_input, proprio_input], axis=0), src_t], axis=0)
