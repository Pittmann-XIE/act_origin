...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names):
        ...
        # Inverse Dynamics Model (Ablation B)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, next_qpos=None, next_image=None, valid_next=None):
        ...
        if actions is not None and next_qpos is not None and next_image is not None:
            ...
            with torch.no_grad():
                ...
                all_cam_pos_target = []
                for cam_id, cam_name in enumerate(self.camera_names):
                    feat_target, pos_target = self.backbones[cam_id](target_image[:, cam_id])
                    all_cam_features_target.append(self.input_proj(feat_target[0]))
                    all_cam_pos_target.append(pos_target[0])
                
                proprio_input_target = self.input_proj_robot_state(target_qpos)
                src_target = torch.cat(all_cam_features_target, axis=3)
                pos_target = torch.cat(all_cam_pos_target, axis=3)
                
                src_target_flat = src_target.flatten(2).permute(2, 0, 1)
                pos_target_flat = pos_target.flatten(2).permute(2, 0, 1).repeat(1, bs, 1)
                additional_pos_embed_target = self.additional_pos_embed.weight.unsqueeze(1).repeat(1, bs, 1)
                pos_target_full = torch.cat([additional_pos_embed_target, pos_target_flat], axis=0)
                
                addition_input_target = torch.stack([latent_input, proprio_input_target], axis=0)
                src_target_full = torch.cat([addition_input_target, src_target_flat], axis=0)
                
                memory_target = self.transformer.encoder(src_target_full, src_key_padding_mask=None, pos=pos_target_full)
