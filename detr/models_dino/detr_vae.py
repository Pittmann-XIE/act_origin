...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names):
        ...
        self.cam_cls_pos_embed = nn.Embedding(len(camera_names), hidden_dim) # learned embed for camera CLS tokens
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None):
        ...
        if self.backbones is not None:
            ...
            if len(all_cls_tokens) > 0:
                # We reuse the 1x1 input_proj Conv2d to efficiently project the 1D CLS vectors
                cls_tokens = [self.input_proj(cls.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1) for cls in all_cls_tokens]
                cls_tokens = torch.stack(cls_tokens, axis=0) # [num_cams, B, hidden_dim]
