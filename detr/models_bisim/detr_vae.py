...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names):
        ...
        self.action_encoder = nn.Sequential(
            nn.Linear(num_queries * state_dim, 256),
            nn.ReLU(),
            nn.Linear(256, 64)
        )
        # Forward Dynamics Model
        self.forward_dynamics_model = nn.Sequential(
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, next_qpos=None, next_image=None, valid_next=None):
        ...
        # ==========================================================
        # Bisimulation Forward Dynamics (Executed only during training)
        # ==========================================================
        bisim_loss = None
        if actions is not None and next_qpos is not None and next_image is not None:
            bs = a_hat.shape[0]
            
            # 1. Extract Current Intention-Conditioned State
            phi_global_current = memory.mean(dim=0) # [Seq, Batch, Dim] -> [Batch, Dim]
            z_theta_current = self.bisim_projection_head(phi_global_current) # Topology Defense
            
            # 2. Encode Action Chunk
            action_flat = a_hat.view(bs, -1)
            action_encoded = self.action_encoder(action_flat)
            
            # 3. Predict Next Latent State
            dynamics_input = torch.cat([z_theta_current, action_encoded], dim=-1)
            pred_z_theta_next = self.forward_dynamics_model(dynamics_input)
            
            # 4. Target Network logic (no_grad)
            with torch.no_grad():
                all_cam_features_next = []
                all_cam_pos_next = []
                for cam_id, cam_name in enumerate(self.camera_names):
                    feat_next, pos_next = self.backbones[cam_id](next_image[:, cam_id])
                ...
                # IMPORTANT: We reuse `latent_input` from the current step to enforce Intention-Conditioned rule
            ...
            # 5. Compute Bisimulation Loss (Masked)
